...
impl<'a> EntityManager<'a> {
    ...
    fn get_mouse_ray(&self, ctx: &mut Context) -> Option<Vec3> {
        ...
        Some(world.truncate(Dim::W).unit_vector())
    }
    ...
    pub fn update(&mut self, ctx: &mut Context<'a>) {
        ...
        let mut closest: Option<(usize, MousePick)> = None;

        if let Some(handle) = self.selected_handle {
            if mouse::button_pressed(ctx, MouseButton::Left) {
                self.entities[handle].clicked(ctx, &self.camera);
            } else {
            ...
            if let Some(hit) = closest {
                ...
                hit.1.entity.mouse_over(ctx, hit.1.point, &self.camera);
                self.selected_handle = Some(hit.0);
